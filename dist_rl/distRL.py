...
class DistanceAgent:
    ...
    def train_policy(self):
        ...
        top_vals_filtered = torch.where(valid_mask, top_vals, torch.tensor(
            float('-inf'), device=top_vals.device))
        ...
        RTG_top_masked = torch.where(
            valid_mask, RTG_top, torch.tensor(0.0, device=RTG_top.device))

        # per-row baseline for stability (advantage-like) - computed only over valid neighbors
        valid_sum = (RTG_top_masked * valid_mask.float()
                     ).sum(dim=1, keepdim=True)
        G_base = valid_sum / valid_counts.float()

        # Zero out invalid scores
        scores = (RTG_top - G_base) * valid_mask.float()
    ...
    def train_policy_reward_only(self):
        ...
        k_per_sample = torch.clamp(
            num_above_threshold, min=5, max=self.top_k)  # [B]
